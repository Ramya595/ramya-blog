...
This article walks through **Brute Force → Better → Optimal** approaches with explanations, real-world examples, and code.
...
The simplest way — check every element one by one.
...
## 2️⃣ Better Approach (Using Built-in Methods)
...
* ⏱️ `O(n)` (but usually slightly optimized internally)
...
> ⚠️ This approach is not optimal for sorted arrays, but it's simpler and slightly better than manual loops.
...
<Steps>
  ...
  <Steps.Step>If target is smaller → search left half</Steps.Step>
  <Steps.Step>If target is larger → search right half</Steps.Step>
...
* ⏱️ `O(log n)` — divides data in half every step
...
* “Guess the Number” game using **higher/lower** logic
